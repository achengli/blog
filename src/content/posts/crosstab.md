...
pubDatetime: 2025-06-08T12:00:00.000Z
...
tags:
  - maths
description: Reimplementación de la función crosstab de GNU Octave para corregir el manejo de NaN.
---
...
$ \left[ x_1^1, x_1^2, \ldots, x_1^n \right] = \left[a_1, a_2, \ldots, a_n\right] $, donde los $x_i^j$ refieren
a las muestras de la variable $X_i$


$$
\forall j \in \{1,\ldots,m\},\; m \in \mathbb{N}
$$


Vamos a construir la matriz $ X\, \in\, \mathbb{R}^{m\times n}$ la cual acoge las 
...
$$
T \in \mathbb{R}^{\times_{i=1}^n \mathrm{length}(U_i)}
$$


Teniendo en cuenta que $\mathrm{length}$ se define cómo


$$
\mathrm{length} : \mathbb{R}^n \rightarrow \mathbb{N}, \quad \mathrm{length}(X) ::= \#_{\mathrm{elem}}(X)
$$
...
$T \in \mathbb{N}^{3 \times 2}$[^1], por lo tanto tenemos una matriz y no una matriz multidimensional.
...
$$
\begin{aligned}
x_1 &= \left[x_1^{(1)}, x_1^{(2)}, \ldots, x_1^{(m)}\right] \\
x_2 &= \left[x_2^{(1)}, x_2^{(2)}, \ldots, x_2^{(m)}\right]
\end{aligned}
$$


Definimos dos matrices $U_1$ y $U_2$ tal que esten compuestas por vectores columna en el caso de
$U_1$ donde cada vector columna sea identico e igual a $\vec{u}_1$, y de vectores fila
para $U_2$ con filas identicas e iguales al vector $\vec{u}_2$


$$
\begin{aligned}
U_1 &= \begin{bmatrix} \vec{u}_1 & \vec{u}_1 & \cdots & \vec{u}_1 \end{bmatrix} \in \mathbb{R}^{m \times m}, &
\vec{u}_1 &= \begin{bmatrix} u_1^{(1)} \\ u_1^{(2)} \\ \vdots \\ u_1^{(m)} \end{bmatrix}
\end{aligned}
$$
...
$$
\begin{aligned}
U_2 &= \begin{bmatrix} \vec{u}_2 \\ \vec{u}_2 \\ \vdots \\ \vec{u}_2 \end{bmatrix} \in \mathbb{R}^{m \times m}, &
\vec{u}_2 &= \begin{bmatrix} u_2^{(1)} & u_2^{(2)} & \cdots & u_2^{(m)} \end{bmatrix}
\end{aligned}
$$
...
Ahora aplicando la diferencia entre los vectores $\vec{x}_1$ y $\vec{x}_2$ tenemos el vector
$\vec{d}$ que permite obtener cuantas veces se encuentra un mismo valor asociado a un 
$d_{i,j} \in D$


$$
\vec{d} = \vec{x}_2 - \vec{x}_1
$$
...
$$
t_{j,k} = \mathrm{count}\left( d_u^{(i)} \triangleq d_{j,k} \right)
$$
...
$$
\begin{aligned}
\mathrm{count} &: (\mathbb{R}^n,\mathbb{R},f:(\mathbb{R}^n,\mathbb{R}) \rightarrow [0,1]) \rightarrow \mathbb{N}, \\
\mathrm{count}(x,y,f) &::= \#_{\mathrm{elem}}(f(x, y) \equiv 1)
\end{aligned}
$$
...
Teniendo los vectores $\vec{x}_1, \vec{x}_2, \ldots, \vec{x}_n$, estos los vuelvo a agrupar en una
matriz $X$
...
\vec{x}_i &= \begin{bmatrix} x_i^{(1)} \\ x_i^{(2)} \\ \vdots \\ x_i^{(m)} \end{bmatrix} \in \mathbb{R}^{m} \\
...
$T \in \mathbb{N}^{\times_{i=1}^n \mathrm{length}(\vec{u}_i)}$, a continuación recorremos $X$ por filas y 
